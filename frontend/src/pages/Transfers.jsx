...
export default function Transfers() {
  ...
  const [form, setForm] = useState({ from_base_id: '', to_base_id: '', equipment_type_id: '', quantity: '', transfer_date: new Date().toISOString().split('T')[0], notes: '' });
  const [saving, setSaving] = useState(false);

  const load = () => {
    setLoading(true);
    api.get('/transfers', { params: filters }).then(r => setRows(r.data)).catch(console.error).finally(() => setLoading(false));
  };

  useEffect(load, [filters]);

  const submit = async () => {
    if (!form.from_base_id || !form.to_base_id || !form.equipment_type_id || !form.quantity) return alert('Fill all required fields');
    if (form.from_base_id === form.to_base_id) return alert('Source and destination must differ');
    setSaving(true);
    try {
      await api.post('/transfers', { ...form, from_base_id: Number(form.from_base_id), to_base_id: Number(form.to_base_id), equipment_type_id: Number(form.equipment_type_id), quantity: Number(form.quantity) });
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <div style={{ fontSize: 11, letterSpacing: '0.3em', color: '#4a5a3a', textTransform: 'uppercase', fontWeight: 900 }}>Logistics</div>
          <div style={{ fontSize: 26, fontWeight: 900, color: '#c8d4b0' }}>Asset Transfers</div>
        </div>
        {canCreate && <button onClick={() => setShowForm(true)} style={{ background: '#4a6a28', border: 'none', borderRadius: 6, color: '#c8d4b0', fontSize: 12, fontWeight: 900, letterSpacing: '0.15em', textTransform: 'uppercase', padding: '10px 16px', cursor: 'pointer' }}>+ New Transfer</button>}
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div style={{ background: '#151910', border: '1px solid #3a4830', borderRadius: 12, width: '100%', maxWidth: 420, margin: 16, padding: 28 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
              <div style={{ fontSize: 16, fontWeight: 900, color: '#c8d4b0' }}>Initiate Transfer</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {[{ k: 'from_base_id', l: 'From Base *' }, { k: 'to_base_id', l: 'To Base *' }].map(({ k, l }) => (
              ...
              {[{ k: 'quantity', l: 'Quantity *', t: 'number' }, { k: 'transfer_date', l: 'Date *', t: 'date' }, { k: 'notes', l: 'Notes', t: 'text' }].map(({ k, l, t }) => (
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
                <button onClick={submit} disabled={saving} style={{ flex: 1, background: '#4a6a28', border: 'none', borderRadius: 6, color: '#c8d4b0', fontSize: 12, fontWeight: 900, letterSpacing: '0.15em', textTransform: 'uppercase', padding: 10, cursor: 'pointer' }}>{saving ? 'Saving...' : 'Confirm'}</button>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #2e3828' }}>
              {['Date', 'From', 'To', 'Equipment', 'Quantity', 'Notes', 'Status'].map(h => (
            ...
            {loading ? <tr><td colSpan={7} style={{ padding: 24, textAlign: 'center', color: '#4a5a3a', fontSize: 13 }}>Loading...</td></tr>
              ...
              : rows.map(r => (
                <tr key={r.id} style={{ borderBottom: '1px solid #1e2418' }}>
                  <td style={{ padding: '12px 16px', color: '#9aaa80' }}>{r.transfer_date?.split('T')[0]}</td>
                  <td style={{ padding: '12px 16px', color: '#9aaa80' }}>{r.from_base_name}</td>
                  <td style={{ padding: '12px 16px', color: '#9aaa80' }}>{r.to_base_name}</td>
                  ...
                  <td style={{ padding: '12px 16px', color: '#9aaa80' }}>{r.notes || '—'}</td>
                  <td style={{ padding: '12px 16px' }}><span style={{ fontSize: 11, fontWeight: 900, letterSpacing: '0.1em', padding: '2px 8px', borderRadius: 4, background: '#1e4a2e', color: '#80d4a0' }}>Completed</span></td>
                </tr>
              ))}
          </tbody>
        </table>
